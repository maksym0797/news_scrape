...
void testOllamaRequest(HttpClient webClient, std::string content)
{
    nlohmann::json body;
    body["model"] = "llama3.2-summarizer";
    body["stream"] = false;
    body["prompt"] = content;

    nlohmann::json result = webClient.post("/api/generate", body);
...
int main()
{
    ...
    testOllamaRequest(webClient, "The quick brown fox jumps over the lazy dog");
